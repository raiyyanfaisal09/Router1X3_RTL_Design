...
function init() {
	...
	for(var table = 0; table < atomic_tables.length; table++) {
		...
		if(curr_table.rows[0].cells[0].innerHTML.search("Weighted Average:") != -1) {
		...
		for(var i = 0; i < headRawCells.length; i++) {
			headRawCells[i].sorttable_sortfunction = (i == 0) ? sort_strings : sort_numbers;
			...
			if (!headRawCells[i].parentNode.to_be_sorted) {
				...
				for(var j = 2; j < curr_table.rows.length; j++) {
			...
			sortnone.id = "sorttable_sortnone_" + curr_table.table_unique_number + "_" + i;
