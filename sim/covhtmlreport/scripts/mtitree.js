...
function Node(id, pid, idx, text, url, popup, icon, title, open) {
	this.id          = id;             // node ID
 ...
 */
Tree.prototype.add = function(id, pid, text, url, popup, icon, title, open) {
	if (this.treeDrawn == false) { // don't accept new nodes once the tree has been drawn
		...
		this.aNodes[idx] = new Node(id, pid, idx, text, url, popup, icon, title, open);
...
Tree.prototype.toHtml = function(node, idx, indent) {
	...
	if (node.url) {
		...
		str += ' class="' + ((this.config.useSelection && node.isSelected) ? 'nodeSel' : 'node') + '"';
		str += ' href="' + node.url + '"';
		...
		str += '<a href="javascript: ' + this.tree + '.o(' + idx + ');" class="node"';
		if (node.popup) str += ' title="' + node.popup + '"';
 ...
 */
Tree.prototype.setOpenStatusAll = function(open) {
	var length = this.aNodes.length;
	for (var idx = 1; idx < length; idx++) {
		if (this.aNodes[idx].hasChildren) {
			this.setOpenStatus(this.aNodes[idx], open);
		}
	}
};
...
Tree.prototype.s = function(idx) {
	if (this.config.useSelection && (this.selectedNode != idx)) {
		if (this.selectedNode != null) {
			document.getElementById("s" + this.tree + this.selectedNode).className = "node";
		}
		document.getElementById("s" + this.tree + idx).className = "nodeSel";
	}
